...
1. After that TCK is passing, initiate a ballot request by creating two https://help.github.com/en/articles/about-pull-requests#draft-pull-requests[draft PRs] requests (also use the `draft` label) against the
...
1. If this new Specification version is targeted for the Platform, update the Jakarta EE API jar by submitting a PR to the https://github.com/eclipse-ee4j/jakartaee-api[jakartaee-api] project that updates the version number of your API jar file.
1. If this new Specification version is targeted for the Platform, update Eclipse GlassFish to use the new version of your API (and implementation, if applicable) by submitting a PR to https://github.com/eclipse-ee4j/glassfish[GlassFish].
...
** After the PRs are sufficiently reviewed and approved, contact the EMO to initiate the official release review ballot by sending an email to emo@eclipse.org.
The `final` and `ballot` labels should be added to the PRs (removing the `draft` label, if it exists).
...
1. A specification committer member adds the `approved` label on the PRs (leaving the `final` and `ballot` labels).
1. The Specification Project's TCK update also needs to be promoted by a member of the specification committee.  Advice on this process can be found in this https://github.com/jakartaee/specification-tools/tree/master/promotion[TCK promotion README].
...
The ballot update can be performed either on the original PR or a subsequent PR, whichever is easiest.
The specification committee will promote the specification project to jakarta.ee by merging the associated PRs into the Jakarta EE Specification Committee project repository.
